...
# OTHER COMMANDS
#
#   ./qemu-secure-lab-1.0.0-EN.sh <command>
#
...
check_other_edition() {
  ...
  for n in lab-libre lab-aislada; do
    if sudo virsh net-info "$n" &>/dev/null; then
      ...
      echo "      ./qemu-secure-lab-1.0.0-ES.sh uninstall"
      echo
      die "Aborted without changing anything."
    fi
  done
...
# Generated by qemu-secure-lab-1.0.0-EN.sh
# Removed with: ./qemu-secure-lab-1.0.0-EN.sh uninstall
#
...
case "${1:-}" in
  ...
  -h|--help|help)
    cat <<EOF
qemu-secure-lab-1.0.0-EN.sh $SCRIPT_VERSION
